...
class Solution {
    ...
    public static int NumberofElementsInIntersection(int a[], int b[], int n, int m) {
        ...
        //adding every elem from array1 into the hashset
        for(int i = 0; i < n; i++){
            set.add(a[i]);
        }
        ...
        //for every elem in array2 we check if it existed in array1
        for(int i = 0; i < m; i++){

            //if it did, we increment counter and remove that key from the set
            if(set.contains(b[i])){
                set.remove(b[i]);
                num++;
            }
